...
<?php
    ...
    if ( isset($_POST) && !empty($_POST)){
        ...
        if ( !isset($_POST["username"]) || empty($_POST["username"])){
            $errors[] = "Username ko hợp lệ ";
        }
        if ( !isset($_POST["password"]) || empty($_POST["password"])){
            $errors[] = "password ko hợp lệ ";
        }
        if ( !isset($_POST["confirm_password"]) || empty($_POST["confirm_password"])){
            $errors[] = "Confirm password ko hợp lệ ";
        }
        ...
        if (empty($errors)){
            /**
             * Nếu ko có lỗi thực thi câu lệnh sql
             */
            ...
            echo "Đăng ký người dùng thành công . Hãy <a href='login.php'>đăng nhập</a>ngay lập tức ";
            echo "</div>";
        }else {
